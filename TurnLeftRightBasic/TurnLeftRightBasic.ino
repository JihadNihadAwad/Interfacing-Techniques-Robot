...
void turnRight() {
  brake();
  reset();     // Reset PID and yaw variables
  delay(20);  // Small delay for stability

  mpuAngleBeforeTurning = getCurrentAngle();  // Record initial angle

  // Start turning right (opposite directions)
  updatePID(turningRPM, 0, 1, 1);  // Opposite and right.
...
void turnLeft() {
  ...
  while (!hasFinishedTurning()) {
    ...
    if (currentTime - previousTimeRotate >= motorRPMCalculationInterval) {
      previousTimeRotate = currentTime;
      calculateRPM();
      updatePID(turningRPM, 0, 1, 0);  // Call updatePID with opposite direction flag.
    }
  }
  brake();
  reset();
