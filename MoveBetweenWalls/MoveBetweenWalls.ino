...
void loop() {
  ...
  // Read the initial front distance (in mm) before moving.
  int initialFront = 0;
  while (!sensor3.isRangeComplete()) {
    initialFront = sensor3.readRangeResult();
  }
  ...
  if (currentTime - previousTimeForward >= motorRPMCalculationInterval) {
    ...
    if (sensor1.isRangeComplete() && sensor2.isRangeComplete()) {
      ...
      sensor1.startRangeContinuous();
      sensor2.startRangeContinuous();
      int diff = rightReading - leftReading;  // positive means left is closer to a wall.
      const float diffThreshold = 15.0;       // threshold in mm (0.5cm), if larger than it we adjust
      float recoveryGain = 0.01;              // scaling factor (higher or lower if it's too weak/strong)

      Serial.print("Difference: ");
      Serial.print(diff);
      Serial.print(", Right Wall is ");
      Serial.println((diff > 0 ? "Further" : "Closer"));
      if (abs(diff) > diffThreshold && abs(diff) < 600.0) {
        //reset();
        if (diff > 0) {

          // If left is too near, reduce left motor speed and slightly increase right motor speed.
          float scale = 1.0 + recoveryGain * diff;
          int rpm1 = constrain((int)(desiredRPM / scale), 30, desiredRPM + 10);
          int rpm2 = constrain((int)(desiredRPM * scale - 5), 40, desiredRPM + 20);
          updatePID(rpm1, rpm2, 1, 1, 0);
        } else {
          // If right is too near, reduce right motor speed and slightly increase left motor speed.
          float scale = 1.0 + recoveryGain * (-diff);
          int rpm1 = constrain((int)(desiredRPM * scale + 10), 40, desiredRPM + 20);
          int rpm2 = constrain((int)(desiredRPM / scale), 40, desiredRPM + 10);
          updatePID(rpm1, rpm2, 1, 1, 0);
        }
      }
    }
